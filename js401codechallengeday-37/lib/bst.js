...
// the first part of our binary search tree should create a class that is the constructor of the tree Node, the basic element of the BST - 
bst.TreeNode = class {
    constructor(value) {
// check if constructors added parameter is a number - for our challenges purposes
        if (typeof value !== 'number')
            throw new TypeError('TreeNode value must be a number');
// the constructor will give us a value, and a left and right value. initializing the value of our parameter,
// setting the left and the right properties to 'null' initially
        this.value = value;
        this.right = this.left = null;
    }
};

// we will now initialize a class within our bst for the actual structure of the tree.  
bst.BinarySearchTree = class {
    // typical validation here, initially setting the root to null before adding a specific tree node.
...
// as always in a constructor, the property is established by setting this.property = property
        this.root = root;
    }


// now if we include an insert method on the binary search tree, can set the initial root of the node
    insert(node) {
        ...
        if (this.root === null)
            this.root = node;
            // if the bst isnt yet empty, we use the helper function below, the helper takes the root and node as arguments. 
        else
            this._insert(this.root, node);
    }

    // insert helper - this will help us insert the node into the
    //correct side of the tree depending on whethere or not the node is large or smaller than the current root
    //the underscore is a coding format convention that helps us to indicate that we are working with a private function.
    _insert(root, node) {
...
//some awesome helpers written by stephen carpenter to help us visualize the tree as we progress
// https://github.com/stevegcarpenter
